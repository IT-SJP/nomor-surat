 ...
 *
 * OTOMATIS:
 * - Script akan auto-create sheet untuk kode perusahaan jika belum ada (misal: CSI)
...
/**
 * Handle GET request - untuk mengambil statistik dashboard
 */
function doGet(e) {
  try {
...
/**
 * Handle POST request - untuk submit form
 */
function doPost(e) {
  try {
...
function saveToSheet(data) {
  try {
    ...
    // Auto-create sheet jika belum ada (misal untuk CSI pertama kali)
    if (!sheet) {
...
function getCompanyStats() {
  try {
    ...
    COMPANY_CODES.forEach((code) => {
      ...
      if (sheet) {
        // Count rows (minus 1 untuk header)
        const lastRow = sheet.getLastRow();
        stats[code] = lastRow > 1 ? lastRow - 1 : 0;
      } else {
        stats[code] = 0;
      }
    });
    ...
    Logger.log("Error getting company stats: " + error);
    throw new Error("Gagal mengambil statistik: " + error);
  }
}
  ...
  try {
    ...
    COMPANY_CODES.forEach((code) => {
      ...
      if (!sheet) {
        ...
        sheet.setColumnWidth(4, 120); // Kode Perusahaan
        sheet.setColumnWidth(5, 120); // Kode Tujuan
        sheet.setColumnWidth(6, 80); // Bulan
        sheet.setColumnWidth(7, 80); // Tahun
        sheet.setColumnWidth(8, 250); // Perihal
        sheet.setColumnWidth(9, 200); // Tujuan
        sheet.setColumnWidth(10, 150); // Letak Arsip
        sheet.setColumnWidth(11, 150); // Requestor
