...
function getToastMixin() {
  ...
  return Swal.mixin({
    ...
    customClass: {
      popup:
        "sjp-toast-popup rounded-xl shadow-xl border border-slate-200/80 dark:border-slate-700/80 font-sans",
...
window.showSwal = function (options = {}) {
  ...
  return Swal.fire({
    ...
    customClass: {
      popup:
        "rounded-3xl shadow-2xl border border-slate-200/80 dark:border-slate-700/80 p-6 sm:p-7 font-sans",
      ...
      confirmButton:
        "btn btn-primary rounded-lg font-bold text-white shadow-md shadow-primary-600/20 text-xs sm:text-sm mx-1",
      cancelButton:
        "btn btn-ghost rounded-lg font-semibold text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 text-xs sm:text-sm mx-1",
    },
...
function createProgressBar() {
  ...
  navProgressBar.className =
    "fixed top-0 left-0 right-0 h-1 bg-gradient-to-r from-emerald-500 via-primary-500 to-teal-400 z-50 transition-all duration-300 pointer-events-none opacity-0";
