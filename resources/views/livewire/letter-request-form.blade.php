<div class="space-y-7 max-w-4xl mx-auto" x-data="{ 
    ...
    <div class="relative overflow-hidden rounded-3xl bg-gradient-to-br from-primary-900 via-emerald-950 to-slate-900 text-white p-6 sm:p-7 shadow-lg border border-primary-500/30">
        ...
        <div class="relative z-10 space-y-4">
            ...
            @if($sub_count > 0 && $previewNumber !== '-')
                <div class="flex items-center gap-2 pt-2 border-t border-white/10 text-xs font-semibold text-emerald-300">
                    <span class="badge badge-xs badge-outline border-emerald-400 text-emerald-300 font-mono">+{{ $sub_count }} Sub-Nomor</span>
                </div>
            @endif
        </div>
    </div>
        ...
        <div class="card bg-base-100 dark:bg-slate-900 shadow-xs border border-slate-200/80 dark:border-slate-800 rounded-3xl p-6 sm:p-8 space-y-6">
            <div class="flex items-center gap-3 pb-5 border-b border-slate-100 dark:border-slate-800/80">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400">Tentukan cabang penerbit, periode waktu, dan tujuan surat resmi</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <!-- Tujuan / Penerima -->
                <div class="col-span-1 md:col-span-2 space-y-1.5" x-data="{ openTargetSuggest: false }" @click.outside="openTargetSuggest = false">
                ...
                <!-- Sub-Nomor Counting Input (Opsional - Berada di bawah field tujuan) -->
                <div class="col-span-1 md:col-span-2 p-4 rounded-2xl bg-slate-50/80 dark:bg-slate-800/50 border border-slate-200/80 dark:border-slate-700/80 space-y-2">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        <div>
                            <div class="flex items-center gap-2">
                                <label class="text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300">
                                    Jumlah Sub-Nomor Surat
                                </label>
                                <span class="badge badge-xs badge-ghost text-[10px] text-slate-400 font-medium">Opsional</span>
                            </div>
                            <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                                Masukkan jumlah sub-nomor jika ingin menerbitkan sub-nomor turunan sekaligus
                            </p>
                        </div>

                        <!-- Stepper Input -->
                        <div class="flex items-center gap-2 self-start sm:self-auto shrink-0">
                            <div class="join border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden shadow-2xs bg-white dark:bg-slate-800">
                                <button
                                    type="button"
                                    wire:click="decrementSubCount"
                                    class="btn btn-sm join-item bg-slate-100 dark:bg-slate-700/60 hover:bg-slate-200 dark:hover:bg-slate-700 border-0 text-slate-600 dark:text-slate-200 px-3 cursor-pointer"
                                    {{ $sub_count <= 0 ? 'disabled' : '' }}
                                    aria-label="Kurangi sub-nomor"
                                >
                                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                <input
                                    type="number"
                                    wire:model.live="sub_count"
                                    min="0"
                                    max="50"
                                    class="input input-sm join-item w-16 text-center font-mono font-extrabold text-sm border-0 bg-transparent text-slate-900 dark:text-white focus:outline-hidden"
                                />
                                <button
                                    type="button"
                                    wire:click="incrementSubCount"
                                    class="btn btn-sm join-item bg-slate-100 dark:bg-slate-700/60 hover:bg-slate-200 dark:hover:bg-slate-700 border-0 text-slate-600 dark:text-slate-200 px-3 cursor-pointer"
                                    {{ $sub_count >= 50 ? 'disabled' : '' }}
                                    aria-label="Tambah sub-nomor"
                                >
                                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div class="modal {{ $showSuccessModal ? 'modal-open' : '' }} z-[100] backdrop-blur-md bg-slate-900/40 dark:bg-slate-950/60" role="dialog">
        <div class="modal-box max-w-lg rounded-3xl border border-slate-200/80 dark:border-slate-800 p-6 sm:p-8 text-center space-y-5 shadow-2xl bg-white dark:bg-slate-900 relative">
            ...
            <div class="space-y-1">
                <span class="badge badge-success badge-soft font-bold uppercase tracking-wider px-3 py-1 rounded-md text-xs">
                    Berhasil Diterbitkan
                </span>
                <h3 class="text-xl sm:text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white mt-1">
            ...
            @if($createdLetter)
                <div class="bg-slate-50 dark:bg-slate-800/60 p-5 rounded-2xl border border-slate-200 dark:border-slate-700/80 space-y-3">
                    ...
                    @if($createdLetter->subLetters->isNotEmpty())
                        <div class="mt-3 pt-3 border-t border-slate-200 dark:border-slate-700/60 text-left space-y-2">
                            <div class="flex items-center justify-between">
                                <span class="text-[11px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">
                                    Sub-Nomor Terbit ({{ $createdLetter->subLetters->count() }}):
                                </span>
                            </div>
                            <div class="space-y-1.5 max-h-36 overflow-y-auto pr-1">
                                @foreach($createdLetter->subLetters as $sub)
                                    <div class="flex items-center justify-between p-2 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-xs">
                                        <span class="font-mono font-bold text-primary-600 dark:text-primary-400">{{ $sub->reference_number }}</span>
                                        <button
                                            type="button"
                                            class="btn btn-xs btn-ghost text-slate-400 hover:text-primary-600 dark:hover:text-primary-400 font-semibold cursor-pointer"
                                            @click="window.copyToClipboard('{{ $sub->reference_number }}', 'Sub-Nomor')"
                                        >
                                            Salin
                                        </button>
                                    </div>
